...
async function checkProbe(ep) {
  ...
  if (r.status >= 200 && r.status < 300) {
    ...
    if (ep.softBeforeWave && WAVE < ep.wave && r.isJson) return record('INFO', ep.method, ep.path, 'probe → 2xx (pre-wave)', r.status, r.text.slice(0, 60));
...
const EP = [
  ...
  { path: 'match_coupons', method: 'POST', wave: 1, tier: 'probe', body: { items: [] }, allow2xx: true },
  ...
  { path: 'smart_match_grocery', method: 'POST', wave: 3, tier: 'probe', body: { items: [] }, allow2xx: true },
