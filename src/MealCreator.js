...
const PROPOSE_WEBHOOK_URL = 'https://n8n-grocery.needexcelexpert.com/webhook/meal_creator_propose';
const BUILD_WEBHOOK_URL = 'https://n8n-grocery.needexcelexpert.com/webhook/meal_creator_build';
const SAVE_WEBHOOK_URL = 'https://n8n-grocery.needexcelexpert.com/webhook/meal_creator_save';
const ADD_TO_WEEK_WEBHOOK_URL = 'https://n8n-grocery.needexcelexpert.com/webhook/call_grocery_agent';
...
const MealCreator = ({ onBack, onNavigate, onToggleSidebar, selectedMeals, setSelectedMeals, debugMode = false }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(PROPOSE_WEBHOOK_URL, {
        method: 'POST',
  ...
  const buildRecipe = async (proposal) => {
    ...
    try {
      ...
      const response = await fetch(BUILD_WEBHOOK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify(payload),
        mode: 'cors',
  ...
  const addToThisWeek = async () => {
    ...
    // Add to local selectedMeals state
    const newMeal = {
      id: Date.now(),
    ...
    // Call the existing webhook to add to weekly_selections
    try {
      const payload = {
      ...
      await fetch(ADD_TO_WEEK_WEBHOOK_URL, {
        method: 'POST',
